...
<input type="file" name="datafile" size="14"><b><font color="#ffffff"> Sortörés <input type="text" name="wrap" value="0" size="1"> karakternél&nbsp;||&nbsp;Alternatív feltöltés:<input type="checkbox" name="method" value="safe"></font></b>
</td>
</tr>
<tr>
...
*/
	...
	//törjük meg a sorokat adott karakternél ha kell
	if($_POST['wrap'] > 0){
	...
	$file1 = fopen('encoded.txt', 'w'); //w=mindig csak egy kód van a fájlban, a=a kovetkezo kódot az előző után rakja
	fwrite($file1,$encoded);
	fclose($file1);
	$file2 = fopen('decoded.txt', 'w'); //w=mindig csak egy kód van a fájlban, a=a kovetkezo kódot az előző után rakja
	fwrite($file2,$decoded);
	fclose($file2);

	//írjuk ki textarea segítségével a kódolt/dekódolt adatokat
	echo '<h2>Kódolva</h2><pre><textarea rows=15 cols=75 readonly>';
	print $encoded;
	echo '</textarea></pre>*<a href=encoded.txt>encoded.txt</a>&nbsp;&nbsp;<font color=#ffffff><i>('.filesize('encoded.txt').'&nbsp;bytes)</i></font><br><br><hr>
		<h2>Dekódolva</h2><pre><textarea rows=15 cols=75 readonly>';
	print $decoded;
