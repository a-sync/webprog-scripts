<?php
//Vec Captcha 1.2 -- Captcha generáló fájl
//(C) 2008 Vector Akashi - Minden Jog Fenntartva!
//www.onethreestudio.com

/* //Vec Captcha 1.2 -- Példa form fájl
	<?php
    session_start();
    ...
    if(md5($_POST['captcha']) == $_SESSION['captcha']) {
      ...
      session_destroy();
			...
			if($_POST['captcha'] != '') { echo 'A megerõsítõkód hibás!<br/>'; }
			echo '
				<form method="post">
...
// ##### Beállítások ##### //
	$img_w = 450;//kép szélessége
	$img_h = 40;//kép magassága

	$chars = 5;//karakterek / számjegyek száma (max. 32)
	$csize = 12;//karakterek mérete

	$str_x = 18;//szöveg pozíciója az X tengelyen
	$str_y = 12;//szöveg pozíciója az Y tengelyen

	$xlines = 2;//teljes vízszintes síkú vonalak száma
	$ylines = 4;//teljes függõleges síkú vonalak száma
	$junks = 2;//véletlen vonaldarabok száma

	$chars_rgb = array(mt_rand(0, 175), mt_rand(0, 175), mt_rand(0, 175));//karakterek / számjegyek RGB színe három elemû tömbben
	$xlines_rgb = array(mt_rand(75, 200), mt_rand(75, 200), mt_rand(75, 200));//teljes vízszintes síkú vonalak RGB színe három elemû tömbben
	$ylines_rgb = array(mt_rand(75, 200), mt_rand(75, 200), mt_rand(75, 200));//teljes függõleges síkú vonalak RGB színe három elemû tömbben
	$junks_rgb = array(mt_rand(75, 200), mt_rand(75, 200), mt_rand(75, 200));//véletlen vonaldarabok RGB színe három elemû tömbben

	$type = 1;//0 = vegyes karakterek beolvasása; 1 = szöveggé alakított számsor beolvasása
